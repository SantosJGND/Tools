...
def extract_profiles(global_data,target_ind_dict):
    ...
    params = {'bandwidth': np.linspace(np.min(global_data), np.max(global_data),15)}
...
def class_and_ideo(Windows_profiles,Out,label_vector,Names_select= [],Comparison_threshold= 3,Outlier_threshold= 1e-4,groups_plot=[0],colors= 'standard',
                  alt_col= []):
    ...
    sub_names= ['sample' + str(focus_indexes[here]) for here in range(len(focus_indexes))]
    if Names_select:
        sub_names= list(Names_select)

    for here in range(len(focus_indexes)):
        Subject = sub_names[here]

...
def get_pop_dict(reference,dir_sim= '',indfile= 'ind_assignments.txt',haps_extract= False, return_inds= False, ind_ext= False):

    if ind_ext:
        ind_assignments= indfile
    else:
...
def ind_assignment_dict(reference,pop_names, pop_lengths,dir_sim= '',indfile= 'ind_assignments.txt', ind_ext= False,
                          min_size= 80, samp= 20, stepup= "increment",outemp= 'ind_assignments{}.txt',write_out= False,haps_extract= True):
    '''
    read ind assignments for a given window; 
    chose one population;
    subset that pop in some way.
    ...
    pop_dict, inds= get_pop_dict(reference,dir_sim= dir_sim,indfile= indfile,
        haps_extract= haps_extract, return_inds= True, ind_ext= ind_ext)
    ...
    for perm in range(samp):
        ...
        for pop_chose in pop_avail:
            ...
            smaller= np.random.choice(pop_list,N,replace= True)
